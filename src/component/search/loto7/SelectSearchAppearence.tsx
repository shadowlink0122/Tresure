...
type SelectSearchAppearenceProps = {
  ...
  canSearch: boolean;
  handleSearchAppearence: (
    params: SearchAppearenceRequestParams,
  ) => Promise<void>;
};

export default function SelectSearchAppearence(
  props: SelectSearchAppearenceProps,
) {
  const loto7Number: number[] = [...Array(MAX_LOTO7_NUMBER)].map(
    (_, i) => i + 1,
  );
  ...
  const handleChangeNumbers = (value: number) => {
    ...
    // すでに含まれる場合、削除
    if (indexOf >= 0) {
      props.setNumbers([...props.numbers].filter((item) => item != value));
      return;
    }
    // 含まれていない場合、追加
    props.setNumbers(
            ...
            <FormControl>
              <Tooltip title="検索対象" arrow disableFocusListener>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    handleChangeIsMainNumber(e.target.value as boolean)
                  }
                >
                ...
                >
                  {
                    ...
                    loto7Number.map((item) => (
                      <MenuItem key={item} value={item}>
                        <Checkbox
                          size="small"
                          checked={props.numbers.indexOf(item) > -1}
                        />
