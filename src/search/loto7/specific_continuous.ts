...
import { continuous, checkContinueNumber } from './continuous';
...
function getSpecificContinuous(
  loto7: Loto7ContinuousNumber[],
  checkNum: number,
) {
  return loto7.filter(
    (item) =>
      item.sameMainNumber.includes(checkNum) ||
      item.sameBonusNumber.includes(checkNum),
  );
}

function main() {
  const fileContent = getSavedLoto7DataSync()!;
  const continuousNumber = checkContinueNumber(
    continuous(fileContent),
    1,
    false,
  );
  for (let i = 1; i < MAX_LOTO7_NUMBER; i += 1) {
    ...
    fs.writeFileSync(join(dir, `${i}.json`), JSON.stringify(result, null, 2));
  }
}
